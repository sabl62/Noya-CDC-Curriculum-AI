...
# Offset between PDF page index and printed page number in the textbook.
...
def pdf_page_to_printed(subject: str, pdf_page: int) -> int:
    """Convert a 1-based PDF page number to the printed textbook page number."""
    offset = PDF_PAGE_OFFSET.get((subject or "").lower(), 0)
    return max(1, pdf_page - offset)
...
def get_pdf_path(subject: str) -> Optional[Path]:
    ...
    if not filename:
        return None
    base_dir = Path(__file__).resolve().parent.parent
    # Check both class_10 subfolder and direct cdc_curriculum folder
    for subdir in [Path("cdc_curriculum/class_10"), Path("cdc_curriculum")]:
        pdf_path = base_dir / subdir / filename
        if pdf_path.exists():
            return pdf_path
    return None
...
def get_chapter_text_for_selection(
    ...
    return (
        f"SCANNED PDF CHAPTER CONTEXT ({pdf_path.name}, pages {pdf_page_to_printed(subject, start_page)}-{pdf_page_to_printed(subject, end_page)}):\n"
        f"{text}\n\n"
...
def get_chapter_pdf_context(subject: str, message: str) -> str:
    ...
    return (
        f"SCANNED PDF CHAPTER CONTEXT ({filename}, pages {pdf_page_to_printed(subject, start_page)}-{pdf_page_to_printed(subject, end_page)}):\n"
        f"{text}\n\n"
        "Use this scanned PDF chapter context as the primary source. "
        "The hardcoded curriculum data is only a locator and support outline."
    )
